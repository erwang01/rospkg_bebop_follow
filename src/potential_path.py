...
class ObstaclePotential:
    ...
    """ keep_out (0,1,2):
        ...
        1 -> forces the gradient away from the center of the obstacle even if the location is inside the wall (radius r), forces the gradient of lines to have a positive y (in the case of m = inf -> positive x)
        2 -> forces the gradient into the center of the obstacle even if the location is outside the wall (radius r), forces the gradient of lines to have a negative y (in the case of m = inf -> positive x)
    """
    def __init__(self, kind=0, location=(0,0), d_safe=1, r=0, m=1, n=1, keep_out=1, max_potential=100, max_gradient=100):
        self.kind_circle=0
        self.kind_line=1 
        if kind in (0,1):
            self.kind = kind
    ...
    def potential(self, location=(0,0), velocity=(0,0)): 
        ...
        if self.kind == self.kind_line:
            self.get_angle(location)
            if self.angle == 0 and self.keep_out == self.keep_out_in:
                return self.max_potential
            elif np.absolute(self.angle) == np.pi and self.keep_out == self.keep_out_out:
                return self.max_potential
            elif self.angle < 0 and self.keep_out == self.keep_out_out:
                return self.max_potential
            elif self.angle > 0 and self.keep_out == self.keep_out_in:
                return self.max_potential
        else:
