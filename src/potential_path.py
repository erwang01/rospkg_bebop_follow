...
class ObstaclePotential:
    ...
    """ #TODO should it be keep_out and keep_in?
    def __init__(self, kind=0, location=(0,0), d_safe=1, r=0, m=0, n=1, keep_out=1, max_speed=1):
        self.kind_circle=0
    ...
    def potential(self, location=(0,0), velocity=(0,0)):
        #TODO Replace max_speed with infinite potential 
        self.distance(location)
        if self.true_dist < self.r and self.keep_out == self.keep_out_out:
            return self.max_speed #TODO make this a mountain ish thing
        elif self.true_dist > self.r and self.keep_out == self.keep_out_in:
            return self.max_speed
        elif self.dist >= self.d_safe:
            return 0
        else:
            return 0.5*self.n*np.square(1.0/self.dist - 1.0/self.d_safe)

    #TODO Implement Velocity
    def gradient(self, location=(0,0), velocity=(0,0)):
        self.distance(location)
        if self.true_dist < self.r and self.keep_out == self.keep_out_out:
            return pol2cart(-1*self.max_speed, self.angle(location, velocity))
        elif self.true_dist > self.r and self.keep_out == self.keep_out_in:
            return pol2cart(self.max_speed, self.angle(location, velocity))
        elif self.dist >= self.d_safe:
            return (0,0)
        else:
            return pol2cart(self.coeff(location)*self.n*((1.0/self.d_safe) - (1.0/self.dist))/(np.square(self.dist)), self.angle(location, velocity))
...
class GoalPotential:
    ...
    def gradient(self, location=(0,0), velocity=(0,0)):
        gradient = 0
        if self.kind == self.kind_quadratic:
            gradient = self._gradient_quadratic(self, location, velocity)
        elif self.kind == self.kind_conic:
            gradient = self._gradient_conic(self, location, velocity)
        else:
            if self.distance(location) > d_threshold:
                gradient = self._gradient_conic(self, location, velocity)
            else:
                gradient = self._gradient_quadratic(self, location, velocity)
        return pol2cart(gradient, self.angle(location, velocity))
...
class MapPotential:
    ...
    def gradient(self, location, velocity=(0,0)):
        ...
        return np.add(self.goal.gradient(location, velocity), gradient_obstacles)
